...
var semantics = grammar.createSemantics().addOperation('parse', {
	...
	ExpressionSelect: (_select, distinct, field, _from, from, _where, where, _order, _by, order, direction, _limit, limit, _offset, offset) => new Expression.Select(
		!!distinct.sourceString,
		...
		{ 'asc': false, 'desc': true }[direction.children[0] && direction.children[0].sourceString],
		_limit.children[0] ? limit.children[0].parse() : null,
		_offset.children[0] ? offset.children[0].parse() : null
	)
});
